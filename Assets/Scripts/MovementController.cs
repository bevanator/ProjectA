...
namespace ProjectA
{
    public class MovementController : MonoBehaviour
    {
        ...
        private void Move()
        {
            ...
            if (_playerInputHandler.Direction.magnitude > 0.1f)
            {
                if (!_isMoving)
                {
                    _isMoving = true;
                    if (IsGrounded) m_PlayerAnimController.StartRunAnim();
                }
            }
            if (!(_playerInputHandler.Direction.magnitude > 0.1f))
            {
            ...
            if (_isMoving)
            {
                ...
                transform.rotation = Quaternion.Euler(0f, _playerInputHandler.RelativeAngle, 0f);
                _characterController.Move(_speed * _playerInputHandler.InputDirection.normalized);
            }
        }
        ...
        private void OnGUI()
        {
            ...
            GUI.Label(new Rect(1200, 150, 300, 300), "Ground Check: " + IsGrounded, style);
